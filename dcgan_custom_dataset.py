...
def load_data(dir_path, img_size=(100, 100)):
	X = []
	for path in paths.list_files(dir_path):
		img = cv2.imread(path)
		img = np.array(img)
		X.append(img)
...
X_train = load_data(NO_DIR, IMG_SIZE)
...
gen = DCGAN.build_generator(16, 64, channels=3)
...
for epoch in range(0, NUM_EPOCHS):
	...
	for i in range(0, batchesPerEpoch):
		...
		if p is not None:
			...
			vis = build_montages(images, (16, 16), (16, 16))[0]
